...
function notifyLoaded() {
	try {
		...
		browser.runtime.sendMessage({ type: "loaded", url: document.URL }).catch(error => {
			// Silently handle extension context errors
		});

		// Send URL of referring page to background script
		browser.runtime.sendMessage({ type: "referrer", referrer: document.referrer }).catch(error => {
			// Silently handle extension context errors
		});
	} catch (error) {
		// Handle synchronous errors (extension context already invalidated)
	}
}
...
function onFocus(event) {
	try {
		browser.runtime.sendMessage({ type: "focus", focus: true }).catch(error => {
			// Silently handle extension context errors
		});
	} catch (error) {
...
function onBlur(event) {
	try {
		browser.runtime.sendMessage({ type: "focus", focus: false }).catch(error => {
			// Silently handle extension context errors
		});
	} catch (error) {
		// Handle synchronous errors
