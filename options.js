...
function initForm(numSets) {
	...
	$("#apiTokenShow").change(apiTokenShow);
	$("#apiAppSecretShow").change(apiAppSecretShow);
	$("#apiTestToken").click(testApiConnection);
...
// Show/hide API token
//
function apiTokenShow() {
	let input = getElement("apiToken");
	let checkbox = getElement("apiTokenShow");
	input.type = checkbox.checked ? "text" : "password";
}

// Show/hide App Secret
//
function apiAppSecretShow() {
	let input = getElement("apiAppSecret");
	let checkbox = getElement("apiAppSecretShow");
	input.type = checkbox.checked ? "text" : "password";
}
...
async function testApiConnection() {
	let testButton = getElement("apiTestToken");
	let statusElement = getElement("apiTokenStatus");
	...
	try {
		...
		const response = await browser.runtime.sendMessage({ type: "testApiConnection" });

		if (response && response.success) {
			statusElement.textContent = "✅ " + response.message;
			statusElement.className = "status-message success";
		} else {
			statusElement.textContent = "❌ " + (response.message || "Connection failed");
...
function disableGeneralOptions() {
	...
	// Disable other items
	let items = [
		"accessPasswordShow", "apiTokenShow", "apiAppSecretShow", "apiTestToken",
