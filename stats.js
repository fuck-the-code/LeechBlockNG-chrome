...
function handleClick(e) {
	...
	if (id == "restartAll") {
		...
		try {
			browser.runtime.sendMessage(message).then(refreshPage).catch(error => {
				console.log("[LBNG] Failed to restart all sets: " + error.message);
			});
		} catch (error) {
			console.log("[LBNG] Error restarting all sets: " + error.message);
		}
		...
		try {
			browser.runtime.sendMessage(message).then(refreshPage).catch(error => {
				console.log("[LBNG] Failed to restart set: " + error.message);
			});
		} catch (error) {
			console.log("[LBNG] Error restarting set: " + error.message);
		}
	}
}
