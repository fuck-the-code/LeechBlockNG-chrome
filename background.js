...
function handleMessage(message, sender, sendResponse) {
	...
	switch (message.type) {
		...
		case "logBlockedWebsite":
			// Log blocked website to external API (handled asynchronously)
			break;

		case "updateRecordWithReason":
			// Update record with user-provided reason (handled asynchronously)
			break;

		case "testApiConnection":
			// Test API connection (handled asynchronously)
			break;

	}
}

// Handle asynchronous messages that need to return responses
//
async function handleMessageAsync(message, sender) {
	switch (message.type) {
		case "logBlockedWebsite":
			// Log blocked website to external API and return record_id
			if (message.url) {
				log("API logging request for URL: " + message.url);
				const recordId = await logBlockedWebsiteWithRateLimit(message.url);
				log("API logging completed, recordId: " + recordId);
				return { recordId: recordId };
			}
			log("API logging failed: no URL provided");
			return { recordId: null };

		case "updateRecordWithReason":
			// Update record with user-provided reason
			if (message.recordId && message.reason) {
				const success = await updateRecordWithReason(message.recordId, message.reason);
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
	// Handle message asynchronously for API calls
	if (message.type === "logBlockedWebsite" || message.type === "updateRecordWithReason" || message.type === "testApiConnection") {
		handleMessageAsync(message, sender).then(sendResponse).catch(error => {
			warn("Error in async message handling: " + error.message);
...
async function getApiConfig() {
	try {
		const result = await gStorage.get({
			apiEnabled: false,
			apiAppId: "",
			apiMultiId: "",
			apiAppSecret: "",
...
async function getValidAccessToken() {
	...
	const tokenData = await gStorage.get({
		accessToken: "",
		tokenExpireTime: 0,
		tokenLastRefresh: 0
	});
	...
	// Check if token exists and is still valid (within 30 minutes)
	if (tokenData.accessToken &&
		tokenData.tokenExpireTime > 0 &&
		tokenData.tokenLastRefresh > 0 &&
		(now - tokenData.tokenLastRefresh) < thirtyMinutes) {
...
// Refresh access token using app credentials
//
async function refreshAccessToken() {
	...
	try {
		...
		if (result.code !== 0) {
			throw new Error(`Token refresh API error: ${result.msg}`);
		}

		const now = Date.now();
		const expireTime = result.expire * 1000; // Convert to milliseconds

		// Store token data with timestamp
		await gStorage.set({
			accessToken: result.tenant_access_token,
			tokenExpireTime: now + expireTime,
		...
		warn("Failed to refresh access token: " + error.message);
		throw error;
	}
}
...
async function testApiConnection() {
	try {
		const apiConfig = await getApiConfig();
		if (!apiConfig.appId || !apiConfig.multiId || !apiConfig.appSecret || !apiConfig.tableId) {
			return { success: false, message: "App ID, Multidimensional ID, App Secret, and Table ID are required" };
		}
		...
		return { success: true, message: "Connection successful! Token is valid and working." };
...
// Log blocked website to external API and return record_id
//
async function logBlockedWebsite(url) {
	try {
		const apiConfig = await getApiConfig();
		log("API config retrieved: " + JSON.stringify({enabled: apiConfig.enabled, appId: apiConfig.appId, multiId: apiConfig.multiId, hasTableId: !!apiConfig.tableId, hasSecret: !!apiConfig.appSecret}));

		// Skip if API logging is disabled
		if (!apiConfig.enabled || !apiConfig.appId || !apiConfig.multiId || !apiConfig.appSecret || !apiConfig.tableId) {
			warn("API logging disabled or missing configuration");
			return null;
		}

		// Validate URL
		if (!url || typeof url !== "string") {
			warn("Invalid URL for API logging: " + url);
			return null;
		}
		...
		// Construct create endpoint URL
		const createUrl = `https://open.feishu.cn/open-apis/bitable/v1/apps/${apiConfig.multiId}/tables/${apiConfig.tableId}/records/batch_create`;

		// Prepare API request
		const requestData = {
			records: [{
				fields: {
					[apiConfig.fieldName]: domain
				}
			}]
		};
		...
		if (!response.ok) {
			warn("API logging failed with status: " + response.status);
			return null;
		}

		const result = await response.json();
		log("API response received: " + JSON.stringify(result));
		if (result.code === 0 && result.data && result.data.records && result.data.records.length > 0) {
			...
			log("Successfully extracted recordId: " + recordId);
			return recordId;
		} else {
			warn("API logging returned unexpected format: " + JSON.stringify(result));
			return null;
		}

	} catch (error) {
		warn("Error in logBlockedWebsite: " + error.message);
		return null;
	}
}
...
async function updateRecordWithReason(recordId, reason) {
	try {
		...
		// Validate inputs
		if (!recordId || !reason || !apiConfig.appId || !apiConfig.multiId || !apiConfig.appSecret || !apiConfig.tableId) {
			return false;
		}
		...
		const updateUrl = `https://open.feishu.cn/open-apis/bitable/v1/apps/${apiConfig.multiId}/tables/${apiConfig.tableId}/records/${recordId}`;
		...
		if (!response.ok) {
			warn("API update failed with status: " + response.status);
			return false;
		}
		...
		warn("Error in updateRecordWithReason: " + error.message);
		return false;
	}
}
...
//
var gApiCallHistory = [];
const API_RATE_LIMIT = 1000; // Maximum 1 call per second

// Check if API call should be rate limited
//
function shouldRateLimitApiCall() {
	const now = Date.now();

	// Remove old entries (older than 1 second)
	gApiCallHistory = gApiCallHistory.filter(timestamp => now - timestamp < API_RATE_LIMIT);

	// Check if we've exceeded the rate limit
	if (gApiCallHistory.length >= 1) {
		return true;
	}

	// Add current timestamp
	gApiCallHistory.push(now);
	return false;
}

// Enhanced logging function with rate limiting
//
async function logBlockedWebsiteWithRateLimit(url) {
	if (shouldRateLimitApiCall()) {
		warn("API call rate limited for: " + url);
		return null;
	}

	return await logBlockedWebsite(url);
}
